    ...
    public static class EkConvertHelper
    {
        ...
        public static EkProduct EkAllegroPlOfferToProduct(Offer offer, decimal exchangeRate)
        {
            ...
            else
            {
                ...
                var price = (P_Price + D_Price) * R_Rate * (decimal)1.25;//(P_Price + D_Price) * (1 + T_Taxes) * (1 + M_Markup) * R_Rate * ExtraRate;
                var finalPrice = CalculatePrice(P_Price, state, product.CategoryId, (P_Price + D_Price) * R_Rate, R_Rate);
                product.Price = RoundPrice(price);
        ...
        private static decimal CalculatePrice(decimal price, EkProductStateEnum state, string categoryId, decimal tempPrice, decimal R_Rate) {
            ...
            if (state == EkProductStateEnum.Used) {
                ...
                {
                    return (tempPrice * (decimal)1.13 + 65 * R_Rate);
                }
                if (price >= 10001)
                {
            ...
            if (state == EkProductStateEnum.New)
            {
                ...
                if (price >= 3001 && price <= 10000)
                {
                    return (tempPrice * (decimal)1.13 + 70 * R_Rate);
                }
                if (price >= 10001)
                {
                    return (tempPrice * (decimal)1.06);
            ...
            return price < 200 ? tempPrice * (decimal)1.3 : tempPrice *(decimal)1.15;
        }
