...
{
    public class ProductSearchInEuropeDataSource : UiBindableObjectCollection<Product>, ISupportIncrementalLoading, IProductSearchDataSource
    {
        public ProductSearchInEuropeDataSource(ProductSearchInEuropeParameters searchParameters, ISearchStateProvider searchStateProvider)
        {
        ...
        private readonly ProductSearchInEuropeParameters _searchParameters;
            ...
            return ThreadHelper.RunInNewThreadAsync(async () =>
                {
                    ...
                    try
                    {
                        ...
                        lock (_currentRequestCancellationTokenSourceLocker)
                        {
                            ...
                            {
                                Log.Error(LogContextEnum.Ui, $"Duplicate invocation of '{nameof(ProductSearchInEuropeDataSource)}.{nameof(LoadNextPageAsync)}'.");
                            }
                        ...
                        for (var i = 0; i < 2; i++)
                        {
                            try
                            {
                                var response = await ServerApiHelper.ProductSearchInEuropeAsync(
                                    new EkKioskProductSearchInEuropeGetRequest()
                                        {
                                            Term = _searchParameters.Term,
                                            TranslatedTerm = _translatedTerm,
                                            CategoryId = _searchParameters.CategoryId,
                                ...
                                _translatedTerm = response.TranslatedTerm;
                            ...
                            {
                                Log.Error(LogContextEnum.Communication, $"'{nameof(ProductSearchInEuropeDataSource)}.{nameof(LoadNextPageAsync)}' failed (try {i + 1}).",
                                    new
                                        {
